    ...
    if weights.endswith(".pt") or weights.endswith(".pth"):
        ...
        if epochs <= start_epoch:
            print('%s has been trained for %g epochs. Fine-tuning for %g additional epochs.' %
                  (opt.weights, (ckpt['epoch'] + 1), epochs))
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if opt.notest is False or epoch == epochs - 1:
            ...
            if opt.savebest is False:
                ...
                ptSaveName = time.strftime("%Y-%m-%d_%H:%M:%S", time.localtime())
                torch.save(save_files, f"./weights/yolo{ptSaveName}.pt")
                weightPathLine = f"./weights/yolo{ptSaveName}.pt"

                weightPath = open("weightPath.txt", mode='w')
                weightPath.write(weightPathLine)
                weightPath.close()
            else:
...
if __name__ == '__main__':
    lastWeightPath = open("weightPath.txt", mode='r')
    lastWeight = lastWeightPath.read()
    lastWeightPath.close()
    ...
    parser.add_argument('--weights', type=str, default=lastWeight, help='initial weights path')
